...
void Coeff::calc_fx(double* x,double Theta) {
	...
	for (int j = 3, i = 1; j < 3 * n - 3; j = j + 3, i++) {
		...
		fx[j + 1] = aB[i] * x[3 * i - 2] + bB[i] * x[3 * i + 1] + cB[i] * x[3 * i + 4] + K1 * x[3 * i + 1] * x[3 * i + 1] - 2.0 * Kb * x[3 * i + 2] - d[3 * i + 1]; // B is depleting at K1* CB*CB
		fx[j + 2] = aY[i] * x[3 * i - 1] + bY[i] * x[3 * i + 2] + cY[i] * x[3 * i + 5] - 0.5 * K1 * x[3 * i + 1] * x[3 * i + 1] + Kb * x[3 * i + 2] - d[3 * i + 2]; // C is generating at half the speed K1* CB*CB
	}
...
void Coeff::calc_jacob(double* x,double Theta) {
	...
	for (int row = 3, i = 1; row < 3 * n-3; row = row + 3, i++) {
		...
		J[row + 1][row - 2] = aB[i];
		J[row + 1][row + 1] = bB[i] + 2 * K1 * x[3 * i + 1];
		J[row + 1][row + 2] = -2.0 * Kb;
		J[row + 1][row + 4] = cB[i];
		//Initialize Species Y;
		J[row + 2][row - 1] = aY[i];
		J[row + 2][row + 1] = 0.5*-2*K1 * x[3 * i + 1];
